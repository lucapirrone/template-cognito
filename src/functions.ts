...
export const preSignUp = async (newUser: SignUpUserEvent, context: any, callback: Callback) => {
  try {
    if (newUser.request.userAttributes.hasOwnProperty("email")) {
      ...
      let existingUser = await getUserByAttribute('email', newUser.request.userAttributes.email);
      if (existingUser) {
        callback("EmailAlreadyExists");
        throw new ApiError("Esiste già un account con questo indirizzo email");
      } else {
        return newUser;
      }
    }
...
export const _preSignUp = async (newUser: SignUpUserEvent, context: any, callback: Callback) => {
  try {
    if (newUser.request.userAttributes.hasOwnProperty("email")) {
      console.log("Richiesta di registrazione per: ", newUser.request.userAttributes.email);
      ...
      if (newUser.triggerSource === 'PreSignUp_ExternalProvider') {
        ...
        let existingUser = await getUserNativeByAttribute('email', newUser.request.userAttributes.email);
        if (existingUser) {
          ...
          if (existingUser.Enabled && existingUser.UserStatus === 'CONFIRMED' &&
            ...
            //TODO bisogna solo linkare newUser a existingUser
            await linkUsers(newUser.userName, existingUser.Username);
            return newUser;
          } else {
          ...
          //TODO bisogna creare l'utente nativo e linkarlo
          var paramsSignUp = {
            ClientId: envNames.clientId,
            Password: generatePassword(),
            Username: newUser.request.userAttributes.email
          };
          var paramsConfirm = {
            UserPoolId: envNames.userPoolId,
            Username: newUser.request.userAttributes.email
          };
          let signUpResponse:any = await cognito.signUp(paramsSignUp);
          console.log("Risposta signup", signUpResponse);
          let newNativeUser = await signUpResponse.send();
          let confirmSignUpResponse = await cognito.adminConfirmSignUp(paramsConfirm);
          await confirmSignUpResponse.send();
...
const getUserNativeByAttribute = (attributeName: string, attributeValue: string) => {
